...
public class hill {
    ...
    public static void setLine(String Line) {
        // this is for scan each line im the text------- 
        Scanner scan = new Scanner(Line);
        String scanPlain = "";
        while (scan.hasNextLine()) {
            scanPlain = scanPlain.concat(scan.nextLine() + "\n");
        }
        Line = scanPlain;
        // this is end of scan -------------------------
        hill.Line = Line;
    ...
    public void matrixtoinvkey(int inv[][], int n) {
       ...
       MyClass.showWErrorDialog("Message", invkey);
    }
    ...
    public boolean check(String key, int len) {
        ...
        d = d % 26;
        if (d == 0) {
            MyClass.showWErrorDialog("Message", "Key is not invertible");
            return false;
